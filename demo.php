...
if(isset($_POST['btnupdate']))
{
    ...
    {
        $img=$_POST['oldimg'];
        $sql="UPDATE `test` SET `red`='$red',`chk`='$checkBox',`name`='$newnm',`image`='$img' WHERE `id`='$id'";
        //echo $sql;
        $res=mysqli_query($con,$sql);
    }
    else
    {
        ...
        if(move_uploaded_file($file_temp,$file_destination))
        {
            $sql="UPDATE `test` SET `red`='$red',`chk`='$checkBox',`name`='$newnm',`image`='$file_destination' WHERE `id`='$id'";
            echo $sql;
            $res=mysqli_query($con,$sql);
        }
    }
...
            <label for="html">HTML</label><br>
            ...
            ?>
            <input type="hidden" name="oldimg" value="<?php echo $img ?>" id="img"><br><br>
